(() => {
  // ../node_modules/.pnpm/@mizchi+selector-generator@1.50.0-next/node_modules/@mizchi/selector-generator/dist/index.js
  ...
  var require_highlight = __commonJS({
    "playwright-src/packages/playwright-core/src/server/injected/highlight.css?inline"() {
      (function() {
        if (!document.getElementById("d921f4ee0d8a705d02fc2c09907b7f4af8c077650d515e9a18c9a8043a5c66a0")) {
          var e = document.createElement("style");
          e.id = "d921f4ee0d8a705d02fc2c09907b7f4af8c077650d515e9a18c9a8043a5c66a0";
          e.textContent = `/**
...
`).replace(/\s\s*/g, " ")).join(" ").trim();
  ...
  function getProps(reactElement) {
    const props = reactElement.memoizedProps || reactElement._currentElement?.props;
  ...
  function buildComponentsTree(reactElement) {
    ...
    const rootElement = reactElement.stateNode || reactElement._hostNode || reactElement._renderedComponent?._hostNode;
  ...
  var kScoreThresholdForTextExpect = 1000;
    ...
    for (let index = 0;index < parts.length; index++) {
      ...
      if (part.name === "internal:control" && part.body === "enter-frame") {
        ...
        if (["xpath", "css"].includes(lastPart.name)) {
          transformed.push(factory.generateLocator(base, "frame-locator", stringifySelector({ parts: [lastPart] })), factory.generateLocator(base, "frame-locator", stringifySelector({ parts: [lastPart] }, true)));
        }
      ...
      if (["xpath", "css"].includes(part.name)) {
        const selectorPart2 = stringifySelector({ parts: [part] }, true);
  ...
  var Highlight = class {
    constructor(injectedScript) {
      ...
      if (typeof this._glassPaneShadow.adoptedStyleSheets.push === "function") {
        ...
        sheet.replaceSync(import_highlight.default);
        this._glassPaneShadow.adoptedStyleSheets.push(sheet);
      } else {
        const styleElement = this._injectedScript.document.createElement("style");
        styleElement.textContent = import_highlight.default;
...
`);
    ...
    async expectSingleElement(element, options) {
      ...
      }
      {
        if (expression === "to.be.in.viewport") {
          ...
          return { received: `viewport ratio ${ratio}`, matches: ratio > 0 && ratio > (options.expectedNumber ?? 0) - 0.000000001 };
        }
      }
      {
      ...
      }
      {
        ...
        if (expression === "to.have.attribute.value") {
          ...
          received = getElementAccessibleName(element, false);
        } else if (expression === "to.have.accessible.description") {
          received = getElementAccessibleDescription(element, false);
  ...
  function asLocator2(lang, selector, isFrameLocator = false) {
    return asLocators2(lang, selector, isFrameLocator)[0];
  }
  function asLocators2(lang, selector, isFrameLocator = false, maxOutputSize = 20, preferredQuote) {
    try {
      return innerAsLocators2(new generators2[lang](preferredQuote), parseSelector(selector), isFrameLocator, maxOutputSize);
    } catch (e) {
      return [selector];
    }
  }
  function innerAsLocators2(factory, parsed, isFrameLocator = false, maxOutputSize = 20) {
    const parts = [...parsed.parts];
    const tokens = [];
    let nextBase = isFrameLocator ? "frame-locator" : "page";
    for (let index = 0;index < parts.length; index++) {
      ...
      if (part.name === "nth") {
        if (part.body === "0")
          ...
          tokens.push([
            factory.generateLocator(base, "nth", part.body)
          ]);
        continue;
      }
      if (part.name === "internal:text") {
        const { exact, text } = detectExact2(part.body);
        tokens.push([factory.generateLocator(base, "text", text, { exact })]);
        continue;
      }
      if (part.name === "internal:has-text") {
      ...
      if (part.name === "internal:has") {
        const inners = innerAsLocators2(factory, part.body.parsed, false, maxOutputSize);
        tokens.push(inners.map((inner) => factory.generateLocator(base, "has", inner)));
        continue;
      }
      if (part.name === "internal:has-not") {
      ...
      if (part.name === "internal:control" && part.body === "enter-frame") {
        const lastTokens = tokens[tokens.length - 1];
        const lastPart = parts[index - 1];
        const transformed = lastTokens.map((token) => factory.chainLocators([
          token,
      ...
      const locatorPart = factory.generateLocator(base, "default", selectorPart);
      if (nextPart && ["internal:has-text", "internal:has-not-text"].includes(nextPart.name)) {
        const { exact, text } = detectExact2(nextPart.body);
        if (!exact) {
          const nextLocatorPart = factory.generateLocator("locator", nextPart.name === "internal:has-text" ? "has-text" : "has-not-text", text, { exact });
          const options = {};
          if (nextPart.name === "internal:has-text")
    ...
    return combineTokens2(factory, tokens, maxOutputSize);
  }
  function combineTokens2(factory, tokens, maxOutputSize) {
    const currentTokens = tokens.map(() => "");
    const result = [];
    const visit = (index) => {
      if (index === tokens.length) {
  ...
  var JavaScriptLocatorFactory2 = class {
    constructor(preferredQuote) {
      this.preferredQuote = preferredQuote;
    }
    generateLocator(base, kind, body, options = {}) {
  ...
  var JsonlLocatorFactory2 = class {
    generateLocator(base, kind, body, options = {}) {
      return JSON.stringify({
        kind,
        body,
  ...
  var generators2 = {
    javascript: JavaScriptLocatorFactory2,
    jsonl: JsonlLocatorFactory2
  };
  function isRegExp2(obj) {
    return obj instanceof RegExp;
  }
  function createSelectorGenerator(self, isUnderTest = false, sdkLanguage = "javascript", testIdAttributeNameForStrictErrorAndConsoleCodegen = "data", stableRafCount = 0, browserName = "Chrome", customEngines = []) {
    const injector = new InjectedScript(self, isUnderTest, sdkLanguage, testIdAttributeNameForStrictErrorAndConsoleCodegen, stableRafCount, browserName, customEngines);
    return (element, opts = {
  ...
  function toLocator(selector, lang = "javascript") {
    return asLocator2(lang, selector);
  }

  // dist/_selector-generator-entry-31716-1770281261823.js
