...
describe('Relay Core Tests', () => {
    ...
    it('should inject script via addScriptTag through CDP relay', async () => {
        ...
        const hasGlobalBefore = await page.evaluate(() => {
            return Boolean(globalThis.__testGlobal);
        });
        ...
        const hasGlobalAfter = await page.evaluate(() => {
            return globalThis.__testGlobal;
        });
    ...
    for (const testCase of accessibilitySnapshotTestCases) {
        it(`should get accessibility snapshot of ${testCase.name}`, async () => {
            ...
            const interactiveData = typeof interactiveResult === 'object' && interactiveResult.content?.[0]?.text
            ...
            const fullData = typeof fullResult === 'object' && fullResult.content?.[0]?.text
    ...
    it('should handle default page being closed and switch to another available page', async () => {
        ...
        expect(resetResult.content[0].text).toContain('Connection reset successfully');
        ...
        expect(initialResult.content[0].text).toContain('first-page');
        ...
        const output = afterCloseResult.content[0].text;
