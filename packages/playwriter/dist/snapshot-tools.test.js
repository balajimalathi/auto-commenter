...
import { setupTestContext, cleanupTestContext, getExtensionServiceWorker, js } from './test-utils.js';
...
describe('Snapshot & Screenshot Tests', () => {
    ...
    it('should show aria ref labels on real pages and save screenshots', async () => {
        ...
        const loadPageWithRetries = async ({ name, url }) => {
            const page = await browserContext.newPage();
        ...
        const browser = await chromium.connectOverCDP(getCdpUrl({ port: TEST_PORT }));
        const withTimeout = async (label, task, timeoutMs) => {
            let timeoutId = null;
        ...
        for (const { name, url, page } of pages) {
            ...
            const { labelCount } = await withTimeout(`showAriaRefLabels(${name})`, async () => {
            ...
            const screenshot = await withTimeout(`screenshot(${name})`, async () => {
            ...
            const labelElements = await withTimeout(`countLabels(${name})`, async () => {
            ...
            await withTimeout(`hideAriaRefLabels(${name})`, async () => {
                await hideAriaRefLabels({ page: cdpPage });
            }, 10000);
            const labelsAfterHide = await withTimeout(`verifyHide(${name})`, async () => {
...
//# sourceMappingURL=snapshot-tools.test.js.map