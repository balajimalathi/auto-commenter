...
export async function startPlayWriterCDPRelayServer({ port = 19988, host = '127.0.0.1', token, logger, cdpLogger, } = {}) {
    ...
    const recordingRelay = new RecordingRelay(sendToExtension, () => extensionWs !== null, logger);
    ...
    async function routeCdpCommand({ method, params, sessionId, source }) {
        switch (method) {
    ...
    app
        ...
        return c.json(Array.from(connectedTargets.values()).map(t => ({
            id: t.targetId,
    ...
    app.get('/cdp/:clientId?', (c, next) => {
        ...
        return {
            ...
            async onMessage(event, ws) {
                ...
                try {
                    const result = await routeCdpCommand({ method, params, sessionId, source });
    ...
    app.post('/recording/start', async (c) => {
        const body = await c.req.json();
        const result = await recordingRelay.startRecording(body);
